 ...
 public:
   int maxTwoSubArrays(std::vector<int> &nums) {
     ...
     while (i != len - 1) {
       ...
       right = maxSubArrayRight(nums, len - 1, i + 1, max_cnt, MIN);
       tmp = left + right;
       if (tmp > max) max = tmp;
       ++ i;
     }
  ...
  inline void update(int &tmp_direct, int &nums, int &max) {
    int tmp = tmp_direct + nums;
    if (tmp_direct > 0) max = max > tmp ? max : tmp;
    else max = max > nums ? max : nums;
    if (tmp > 0) tmp_direct = tmp;
    else tmp_direct = 0;
  }
 };
